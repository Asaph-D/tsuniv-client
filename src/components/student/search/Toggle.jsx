...
const ToggleSwitch = () => {
    // Select state and action separately to avoid creating a new object on every render.
    const isCertified = useStudentSearchStore(state => state.isCertified);
    const toggleCertified = useStudentSearchStore(state => state.toggleCertified);
    ...
    return (
        <label className="toggle text-base-content">
            <input
                type="checkbox"
                checked={!isCertified}
                onChange={toggleCertified} // Call the store action directly
            />
